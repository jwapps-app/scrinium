...
- **2026-07-11 (share links, notes, tag colors — migration 0011):** **Share links:** `POST /documents/{id}/share` mints a `secrets.token_urlsafe` token (default 7 days, 0 = no expiry, 365 max); public `GET /api/share/{token}` + `/file` serve exactly that one document's minimal metadata and file with no auth — expired/revoked/trashed all read as the same 404. Owner can list active links and revoke all. Frontend `/share/{token}` is a minimal public viewer (title, download, PDF); document ⋯ menu gains "Copy share link" (clipboard) and "Revoke share links". **Notes:** freeform text on the document (outside OCR text), edited in the details strip, saved on blur; deliberately not in FTS for now. **Tag colors:** color picker per tag in Settings, tinted chips in the details strip, color dots in the sidebar tree. Verified: public page renders without auth, revoke → 404, note persisted from UI, chip + dot show the picked color.

- **2026-07-11 (chunked uploads):** Big files now clear the tunnel's 100 MB request cap: the web app uploads anything over 80 MB as a session of 32 MB parts (`POST /documents/uploads` → `PUT …/{n}` → `…/complete` assembles and runs normal intake, dedup included), with per-file progress shown in the library. Sessions are tenant-scoped; abandoned ones are swept by the worker after 24 h. Verified: 70 MB file in 3 chunks, reassembled byte-identical (sha256).
- **2026-07-11 (runtime OCR engine toggle):** The engine is now switchable from Settings without a redeploy: an `app_settings` override (`ocr_engine_override`) wins over the `OCR_ENGINE` env default and is read per job, so the change applies from the next claimed document. Settings shows Tesseract / Apple Vision buttons (Apple disabled until `APPLE_OCR_URL` is configured; a hint appears when overriding the server default; choosing the default clears the override). Apple still falls back to Tesseract on sidecar failure. Verified live: override → doc OCR'd by tesseract; clear → next doc back on apple; bad values 400.

- **2026-07-11 (Paperless importer, library export, bulk metadata):** **Paperless-ngx importer:** drop the `document_exporter` output (folder or zip) into `DATA_DIR/import/`, hit Import in Settings. Parses the Django-fixture manifest defensively — titles, created dates (→ doc date), tags (hex colors *and* legacy palette ints), correspondents, types, notes all carry over; originals go through normal intake, so dedup makes re-runs safe and skip-mode OCR keeps Paperless's existing text layers. Background task, progress polled from Settings. Verified with a synthetic export: both docs imported with full metadata; re-run skipped both. **Library export:** one click writes `DATA_DIR/export/library-export-<stamp>.zip` — every original, every archive, and a manifest.json of all metadata (tags, correspondents, types, dates, notes, custom fields). Server-side, so size doesn't matter; verified 19 docs / 547 MB. Documents are never hostage to the app. **Bulk metadata:** bulk bar gains From and Type menus (set or clear correspondent/doc type across a selection); verified set + clear.
...
- **2026-07-11 (test suite + CI gate):** First automated tests: `backend/tests/` — 45 tests in ~1s. Unit coverage for the pure logic (date extraction incl. DATE_ORDER/plausibility, OCR remedy-chain construction + escalation/unfixable gating + text-only rescue, separator-value matching, importer manifest parsing (hex + legacy colors), watch-folder skip rules and retention sweep, blob-store roundtrip) and API coverage against a real migrated Postgres (`scrinium_test`, dropped/recreated per run, real alembic migrations): auth, upload/dedup/415, full trash lifecycle, tag hierarchy + cycle guard + colors, bulk actions incl. set/clear correspondent, notes, share-link lifecycle incl. trashed-doc 404, rules classify, saved views, custom fields, page ops + guardrails, engine-toggle guards, insights shape. **CI now gates publish:** a `test` job (Postgres service container) must pass before images build — a red test can no longer reach the NAS via Pull-and-redeploy. Local run: `python -m venv backend/.venv && pip install -r requirements*.txt && pytest` (needs the dev postgres on :5434).

- **2026-07-11 (overlooked-features round):** **User management** — the gap: no way existed to change a password or add a user. Now: `POST /auth/change-password` (verifies current), `GET/POST/DELETE /auth/users` (same tenant, can't remove yourself), Settings "Account" card. **Page-edit undo** — rotate/delete-pages no longer destroy the old blobs: they transfer to a "(before page edit)" snapshot document born in the trash — restorable for the retention window, auto-purged after, zero extra disk. **Reading position** — the viewer remembers where you stopped (localStorage per doc/device) and resumes there on open; saved from the existing scroll anchor. **PDF outline** — a Contents menu appears when the PDF carries bookmarks (flattened ≤2 deep, page-resolved). **Night mode** — ☾ toggle inverts page pixels only. **Merge** — bulk-bar action concatenates 2–50 selected PDFs into a new document (order as selected); sources go to the trash. **Queue-drained push** — when a backlog ≥10 finishes, one "all caught up" notification. **Scheduled exports** — `EXPORT_EVERY_DAYS` (0=manual) auto-runs the library export from the worker, keeping `EXPORT_KEEP` (3) newest zips. **Health strip** — Settings shows worker liveness (15s pulse in app_settings), queue depth, disk free at a glance. Tests grew 45→51 (user mgmt roundtrip, merge, undo-snapshot restore + file integrity, health shape) — features now ship with tests.

- **2026-07-11 (deferred trio: text selection, near-dups, weak OCR):** **Selectable text** — the viewer overlays pdf.js's TextLayer on every rendered page: select and copy the OCR text under the cursor; failures cost selection only, never the render; highlight marks sit above the layer. **Near-duplicate detection** — `documents.simhash` (migration 0013): a 64-bit word-bigram simhash stamped at OCR completion (and capture ingest), lazily backfilled 300/pulse by the worker. `GET /insights/duplicates` finds pairs via 4×16-bit band bucketing + Hamming ≤6 — sub-second at 13k docs, no quadratic scan. Catches the same document scanned twice (byte-identical is already blocked at ingest); Insights lists pairs with similarity %. Texts under ~20 words fingerprint as 0 (excluded — too short to mean anything). **Weak OCR list** — Insights flags ready documents yielding <150 chars/page: OCR "succeeded" but produced almost nothing, usually a bad scan. Tests 51→57 (simhash properties incl. OCR-noise tolerance on realistic-length text, banding correctness, endpoint pair detection); verified live: bigB≈bigC found at 100%, page-edited bigA correctly no longer matches, text selection returns real strings.

- **2026-07-11 (polish + steady sidebar):** **Sidebar jump fixed** — between jobs the running list momentarily emptied and at the end it toggled, bouncing the whole sidebar. Now the processing panel has slot memory (keeps as many bar rows as the recent maximum while the queue is active, padding with pulsing "next file…" placeholders) and a linger (after draining it shows "All caught up" and leaves once, ~15s later — never flickers off/on). Verified under churn: 8 fast jobs, panel height byte-stable for the whole run, then a single clean exit. **Beautify pass:** design tokens (shadows, ok/warn colors); status chips rebuilt on color-mix so they finally look right in dark mode (were hardcoded light pastels); focus-visible rings everywhere; button hover/active states; frosted (backdrop-blur) sticky viewer bar; animated progress fills; pill count badges in the sidebar; thin quiet scrollbars; `prefers-reduced-motion` respected; slightly warmer light palette and deeper dark palette.

- **2026-07-11 (working-library round — migration 0014):** **Highlights & annotations:** select text in the viewer (the new text layer) → floating "Highlight" → saved with page + page-fraction rects, drawn at any zoom; ✺ drawer lists them with editable notes, jump-to-page, delete; `GET /api/annotations?q=` searches your own markings across the library. **Synced reading positions:** per-user server-side (reading_positions), debounced saves; server position wins over the device-local one on open. **Related documents:** fingerprint proximity (Hamming ≤26) strip on the doc page — catches excerpts, rescans, revisions ("Shares content with: …"). **Duplicate resolution:** Trash left / Trash right / Not a dupe per pair; dismissals persist (dismissed_duplicates) so pairs stop resurfacing. **Review mode:** `/review` deals ready-but-unfiled docs (no correspondent AND no type) one at a time — details strip inline, ←/→/X/O keys; sidebar shows "To review (N)". **Keyboard shortcuts:** `/` search, `g`+`l/r/i/s` nav, `?` help overlay. **Offline documents:** "Keep offline" caches the file + metadata via the Cache API; the viewer falls back to the cache when the server is unreachable; `/offline` lists the stash; the service worker now keeps the app shell loadable offline (network-first navigations with cached fallback, cache-first hashed assets, /api untouched). Deliberately a go-bag, not a mirror — the OS may evict PWA storage. Tests 57→62 (annotation lifecycle+validation, position sync, related, dismiss persistence, review bucket).

- **2026-07-12 (browsable exports):** The library export is no longer a flat UUID dump: documents land in folders built from their **tag hierarchy** (deepest chain wins, alphabetical on ties; `Untagged/` otherwise) with sanitized human titles and real extensions — `originals/Taxes/2023/W2.pdf` plus a parallel `searchable/` tree of the OCR'd copies and the same manifest.json. Since folder drops became tag chains at ingest, an export now reconstructs the structure the files arrived in — a true round trip. Windows-safe names, per-folder collision suffixes. Verified: nested chains (Construction/Architecture, Insurance/Auto) in a live 574 MB export; tests 62→64 (sanitizer, deepest-chain selection, zip layout).

- **2026-07-12 (exports at scale):** Two formats, chosen at export time (env `EXPORT_FORMAT` for scheduled runs; default **folder**). **Folder** writes a real browsable tree into the export share — and because blobs and exports share a volume, every file is a **hardlink**: a multi-hundred-GB library exports in seconds using no additional disk, and either copy survives the other's deletion. **Zip parts** (`part_gb`, default 10) packs portable archives that split **only at tag-folder boundaries** — every part carries complete subtrees under the same root, so unzipping all parts into one directory reassembles the tree exactly; only a single folder larger than the cap spans parts (identical paths, still reassembles). manifest.json rides in part 1 / tree root. Rotation prunes whole runs (folder or all parts of a stamp) beyond `EXPORT_KEEP`. Verified: live folder export 41/41 hardlinked with nested tag folders; part-planner unit tests (folders kept whole, oversized folder split without loss).

- **2026-07-12 (tags page + UI feedback round):** **Tag management moved to `/tags`** (Settings links to it) — with a real library it's hundreds of rows and deserves the room; rows now wrap cleanly on mobile (full-width selects, card-like layout). **Auto-color** (`POST /tags/auto-color`): root tags get distinct hues via golden-angle spacing (any count stays separated), subtags become progressively lighter shades of their root's hue with slight sibling variation; manual tweaks afterwards stick. **Sidebar scroll cue**: scrolling shadows (background-attachment local/scroll technique) — soft edges appear only when content continues past the fold, so the cut-off no longer looks like the end. **Sticky library toolbar**: search/filters stay pinned (frosted) while scrolling the grid. Root cause found on the way: `overflow-x: hidden` on body (the mobile side-scroll fix) silently breaks `position: sticky` — replaced with `overflow-x: clip`, which clips without creating a scroll container. Verified: sticky at scrollY 600 with no side-scroll regression; live auto-color (Construction green → Architecture lighter green, roots visibly distinct).

- **2026-07-12 (auto-color v2 — field report):** v1 gave distinct hues only to root tags; John's tree has ONE root ("Prepper Docs") over ~20 branches, so everything came out shades of red — practically worthless. Rewritten as **weighted hue-range partitioning** (`services/palette.py`, pure + unit-tested): every node owns a wheel slice and splits it among children **proportionally to subtree size**, recursively. Hue variety therefore flows to wherever the tree actually fans out, at any depth, and stray leaf roots (the auto-created Email tag) can't steal wheel space from the main tree. Children of a dominant root span nearly the full wheel; descendants stay inside the family slice, lighter with depth; single-child chains pass their whole slice down. Verified live: single-root sim went from 8 near-identical purples to green→teal→blue→violet→magenta with the sub-tag a lighter family shade. Tests: fan-out spread, slice containment, stray-roots-don't-steal.

- **2026-07-12 (trust & safety batch — migration 0015):** **TOTP two-factor:** RFC-6238, dependency-free (SHA-1/6-digit/30s, ±1 step drift), per-user. Enroll in Settings → Account (QR + manual key, code-verified before it arms); login then requires the code (`totp_required` two-step in the UI); disabling needs password + current code — lost device means direct DB access, stated honestly in the UI. **Restore from our own export:** the Import button now auto-detects manifest format (Paperless fixture list vs Scrinium dict) — a Scrinium export restores titles, dates, tags, correspondents, types, notes, and pulls text from the exported searchable copies so restored docs are searchable immediately without re-OCR (archives rebuildable via Re-OCR). This closes the disaster-recovery loop: export + fresh install + Import = your library back. **Expiration dates:** `documents.expires_on` (details strip field), sidebar "Expiring soon (N)" (≤60 days) with `expires` sort, and a once-daily push when documents lapse within 30 days. **Zip downloads:** bulk-bar Download — selected docs, or (with Entire filter on a tag) everything under a tag with tag-path folders; searchable copies preferred, 4 GB guard. **Share-links overview:** Settings lists every active public link with copy/revoke — the audit surface for exposure. **Did-you-mean:** on zero-hit single-word searches, pg_trgm `word_similarity` against titles/tags/correspondents offers clickable corrections. Tests 71→78.
...
- **2026-07-12 (mission batch — migration 0016):** **OCR upgrade advisor:** Settings counts documents that finished on Tesseract (helper down at the time) and one click queues them all for Apple re-OCR at **low priority** (`jobs.priority`, claim order priority-then-age) — upgrades only run when the queue is otherwise idle. **Scoped search:** `?tag_id=` on /search; the UI searches within the active tag filter ("…in Medical"). **Binder builder:** bulk-bar action producing one print-ready PDF — generated cover + table of contents (sheet numbers account for front matter; drawn via Ghostscript from PostScript, zero new deps) + every selected document (or a whole tag). The paper tier below the offline app. **Integrity checking:** hourly sweep re-hashes 300 blobs (oldest verification first — full store cycles in days) against stored sha256; `blobs.verified_at`; health strip shows "Integrity N/N verified" or flags corruption loudly. **Text-native ingestion:** .txt/.md/.epub/.docx/.xlsx/.pptx/.odt accepted everywhere (upload, watch, chunked); text extracted natively at intake (office formats are zipped XML — stdlib only, no LibreOffice), docs land READY instantly with engine `native`; new serif **reader view** renders them in-app (no fake PDF conversion — download the original for exact formatting); library cards show a file-type badge. Found & fixed en route: tagless ingest left the tags collection unloaded → MissingGreenlet in classify (latent since the capture path). Tests 83→89 (one gs-gated; CI now installs ghostscript).

- **2026-07-12 (security audit + hardening):** Adversarial pass (two focused sub-audits + direct review). Fixed: **(HIGH) import path traversal** — a malicious Paperless/Scrinium export manifest could reference `/etc/passwd` or `../` and pull server files into the library; now every manifest path is confined to the export dir (`_safe_member`). **(MED) decompression bombs** — office text extraction `zf.read` and import `extractall` now check uncompressed size before reading (per-member 200 MB, per-doc 1 GB, extract 100 GB). **(MED) bulk cross-tenant FK** — `set_correspondent`/`set_doc_type` now validate the id belongs to the tenant (matched the single-doc path). **(MED) auth brute force** — in-process rate limiter on login (10/min) and setup (5/min); login now runs a dummy bcrypt for missing users (no timing enumeration). **(LOW) XML hardening** — defusedxml for office XML (entity/XXE proof regardless of libexpat); external-entity read confirmed blocked. **(LOW) inline serving** — `/file` and public `/share/*/file` serve `inline` only for pdf/images, `attachment` otherwise, always `X-Content-Type-Options: nosniff`. **(LOW) nginx security headers** — CSP (all same-origin; verified the built PWA + pdf.js worker load clean under it), X-Frame-Options SAMEORIGIN, Referrer-Policy, nosniff. **(LOW) consumed-copy deletion** prefix-containment bug fixed; **chunked upload** size caps (128 MB/chunk, 40 GB/session). **Verified clean:** JWT (HS256 pinned, exp enforced, live-user re-resolved so deleted users' tokens die), tenant checks on all non-bulk id handlers, share tokens (192-bit, leak nothing beyond the one doc), TOTP (no bypass, compare_digest), subprocess calls (all list-form, no shell, no filename-as-flag), export sanitize/blob-store paths, no DB/api ports published (nginx-only). **Deliberately deferred:** non-root container (would break the existing bind-mounted /data ownership on the NAS — documented in Dockerfile; opt-in for shared deployments); device-token reassignment (correct for legit phone-switching on a single-user box). Tests 89→97 (8 security regressions).
